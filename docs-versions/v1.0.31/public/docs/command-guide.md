...
Binding a repo does not expose any existing vault item by itself. Use `hasp secret expose NAME --project-root <repo>` when that repo needs an existing secret.
