...
- Supersede v0.1.55 with hosted public release publication fixed end to end:
  the R2 mirror publisher now uploads from an absolute release directory, so
  AWS CLI `put-object --body` receives real file paths after the script changes
  into the release tree.
- Keep the v0.1.55 trusted GPG fingerprint workflow wiring, v0.1.54 hosted
  Linux coverage portability, v0.1.50 daemon leak prevention, public mirror,
  docs, and release-gate improvements in the final release set.

## [v0.1.55]

- Supersede v0.1.54 with the public Release workflow passing the trusted GPG
  fingerprint allowlist into artifact packaging, so hosted publication rejects
  unknown signing keys without rejecting the configured HASP release key.
- Keep the v0.1.54 hosted-Linux coverage portability, v0.1.50 daemon leak
  prevention, public mirror, docs, and release-gate improvements in the final
  release set.

## [v0.1.54]

- Supersede v0.1.53 with OS-portable coverage for hosted Linux public-export
  release gates: clipboard command selection, gitsafe cache miss/eviction,
  non-Darwin keyring fallback, and explicit Store keyring construction are now
  covered without relying on the local Darwin runner.
- Keep the v0.1.53 runner-portable release-publication scanner, coverage
  diagnostics, v0.1.52 deterministic process-test fix, v0.1.50 daemon leak
  prevention, public mirror, and docs improvements in the final release set.

## [v0.1.53]

- Supersede v0.1.52 with runner-portable release-publication workflow scanning
  so hosted release gates do not depend on `rg` inside the clean public export.
- Add direct Unix daemon stop wait-helper coverage and coverage-failure
  diagnostics so Linux hosted release gates can prove the same 100% statement
  coverage as the local gate.
- Keep the v0.1.52 deterministic process-test fix, v0.1.51 runner-portable
  public-export scanner, v0.1.50 daemon leak prevention, packaged eval daemon
  parent guard, deterministic root leak reporting, hosted-CI, coverage, Darwin
  assurance, deterministic race, public mirror, and docs improvements in the
  final release set.
