...
Enabling the trusted-harness catalog is only the first gate. Destructive and
binding mutations such as `hasp_secret_expose`, `hasp_secret_hide`, and
`hasp_secret_delete` also require an explicit `hasp session grant-mutation`
for the named item and action.
...
Please expose that item with hasp secret expose OPENAI_API_KEY --project-root <repo>
or give me a different target.
```
