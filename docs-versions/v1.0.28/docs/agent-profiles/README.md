...
Default MCP configs cannot auto-expose secrets and do not include mutation
tools. If a trusted local harness intentionally enables
`HASP_MCP_ENABLE_UNSAFE_SECRET_WRITE_TOOLS=1`, exposure/hide/delete still
requires a matching `hasp session grant-mutation` from the operator.
