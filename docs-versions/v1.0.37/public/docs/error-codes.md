...
- if the hint says a reference is not exposed, run
  `hasp secret expose <NAME> --project-root <repo>`; binding the repo alone is
  not enough
- if the secret truly does not exist, create it with `hasp secret add <NAME>`
...
- expose an existing vault item explicitly with
  `hasp secret expose <NAME> --project-root <dir>`
...
- expose an existing vault item to the repo with
  `hasp secret expose <NAME> --project-root <repo>`; `hasp project bind` alone
  does not expose existing vault items
