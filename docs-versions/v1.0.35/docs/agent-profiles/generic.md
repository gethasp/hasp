...
Default MCP configs cannot auto-expose existing vault items. If an agent finds
that `@NAME` exists but is not available in the project, an operator should run
`hasp secret expose NAME --project-root <repo>`. Trusted local harnesses that
enable unsafe mutation tools still need `hasp session grant-mutation` before
expose, hide, or delete can succeed.
...
- `hasp_targets` and `hasp_target_explain` can inspect value-free manifest
  targets when `.hasp.manifest.json` is present
...
- `hasp_run`
- `hasp_inject`
- Prefer `hasp_run` or `hasp_inject` with a manifest `target` when the repo has
  `.hasp.manifest.json`; call `hasp_targets` before raw mappings.
- If no manifest exists and local edits are allowed, use
  `hasp template add <target> --env NAME=@NAME -- <command>` to create a
  value-free template. Do not put raw values in `.hasp.manifest.json`.
