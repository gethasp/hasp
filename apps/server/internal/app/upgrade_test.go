...
func TestUpgradeCommandRefusesWithoutPinnedKeys(t *testing.T) {
	...
	deps := defaultUpgradeDeps()
	err := upgradeCommandWithDeps(context.Background(), []string{"--version", "1.1.0", "--yes"}, bytes.NewReader(nil), io.Discard, io.Discard, deps)
...
func TestUpgradeCommandRefusesDowngrade(t *testing.T) {
	t.Setenv("HASP_VERSION", "1.1.0")
	restore := release.SetPinnedKeysForTest(hex.EncodeToString(newPinnedKeyForTest(t)))
	defer restore()

	deps := defaultUpgradeDeps()
	err := upgradeCommandWithDeps(context.Background(), []string{"--version", "1.0.0", "--yes"}, bytes.NewReader(nil), io.Discard, io.Discard, deps)
...
func TestUpgradeCommandRefusesNonInteractiveWithoutYes(t *testing.T) {
	...
	err := upgradeCommandWithDeps(context.Background(), []string{"--version", "1.1.0"}, bytes.NewReader(nil), io.Discard, io.Discard, deps)
...
func TestUpgradeCommandHappyPathYes(t *testing.T) {
	...
	if err := upgradeCommandWithDeps(context.Background(), []string{"--version", "1.1.0", "--yes"}, bytes.NewReader(nil), &stdout, io.Discard, deps); err != nil {
		t.Fatalf("upgrade: %v", err)
	}
	if !called {
		t.Fatal("expected release.Upgrade to be invoked")
	}
	if !strings.Contains(stdout.String(), "Upgraded 1.0.0 → 1.1.0") {
...
func TestUpgradeCommandJSONOutput(t *testing.T) {
	...
	var stdout bytes.Buffer
	if err := upgradeCommandWithDeps(context.Background(), []string{"--version", "1.1.0", "--yes", "--json"}, bytes.NewReader(nil), &stdout, io.Discard, deps); err != nil {
		t.Fatalf("upgrade: %v", err)
	}
	out := stdout.String()
	for _, want := range []string{`"from_version"`, `"to_version"`, `"signer_key"`, `"1.0.0"`, `"1.1.0"`} {
...
func TestUpgradeCommandInteractiveDeclineAborts(t *testing.T) {
	...
	err := upgradeCommandWithDeps(context.Background(), []string{"--version", "1.1.0"}, strings.NewReader("n\n"), io.Discard, &stderr, deps)
...
func TestUpgradeCommandInteractiveAcceptInvokesUpgrade(t *testing.T) {
	...
	var stdout bytes.Buffer
	if err := upgradeCommandWithDeps(context.Background(), []string{"--version", "1.1.0"}, strings.NewReader("y\n"), &stdout, io.Discard, deps); err != nil {
		t.Fatalf("upgrade: %v", err)
	}
...
func TestUpgradeCommandUnknownExtraArgs(t *testing.T) {
	...
	deps := defaultUpgradeDeps()
	err := upgradeCommandWithDeps(context.Background(), []string{"--version", "1.1.0", "stray"}, bytes.NewReader(nil), io.Discard, io.Discard, deps)
