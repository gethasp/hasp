...
func TestUpgradeRefusesDowngrade(t *testing.T) {
	opts, _, _, _ := fixture(t, "1.1.0", "1.0.0")
...
func TestUpgradeRefusesTamperedTarball(t *testing.T) {
	opts, fh, _, _ := fixture(t, "1.0.0", "1.1.0")
...
func TestUpgradeProgressOutput(t *testing.T) {
	...
	out := buf.String()
	for _, want := range []string{"Verified target 1.1.0", "Verified KEYS file", "Verified tarball signed by", "Replaced"} {
