...
You put secrets into the vault. You bind a repo. A
*[committed value-free manifest](value-free-manifests.md)* can describe
requirements and targets. You connect the app or agent that works inside that
repo. HASP gives the app or agent a reference, then resolves that reference at
execution time.
...
`server.integration`, or `release.sign`.
...
access, store values, or make the whole project secret set available. Read
[Value-free manifests](value-free-manifests.md) for the full contract.
